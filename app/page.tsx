...
export default async function Home() {
  ...
  // Fetch latest 6 products for the Store Section
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white selection:bg-[#14a800] selection:text-white font-sans">
          ...
          <div className="flex-1 space-y-8 text-center md:text-left">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-8 pt-4">
              <div className="text-center md:text-left">
                <div className="text-3xl font-bold text-white">{content?.statNumber || "100%"}</div>
                <div className="text-xs text-gray-500 uppercase tracking-widest font-semibold">{content?.statLabel || "Success"}</div>
              </div>
              <div className="w-px h-12 bg-white/10 hidden md:block"></div>
              <div className="text-center md:text-left">
                <div className="text-3xl font-bold text-white">94+</div>
                <div className="text-xs text-gray-500 uppercase tracking-widest font-semibold">Jobs Done</div>
              </div>
              <div className="w-px h-12 bg-white/10 hidden md:block"></div>
              <div className="text-center md:text-left">
                <div className="text-3xl font-bold text-white">{content?.hourlyRate || "$50/hr"}</div>
                <div className="text-xs text-gray-500 uppercase tracking-widest font-semibold">Rate</div>
              </div>
            </div>
      ...
      {/* --- SECTION 2: PREMIUM STORE (High Priority) --- */}
      <section id="store" className="py-24 px-6 border-y border-white/5 bg-[#080808] relative z-10">
              ...
              {latestProducts.map((product) => (
                <div key={product.id} className="group bg-[#0e1011] border border-white/10 rounded-2xl overflow-hidden hover:border-[#14a800]/50 transition-all duration-300 flex flex-col">
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    ...
                    {/* EXCERPT FIX: Strip HTML tags for clean display */}
        ...
        <div className="max-w-7xl mx-auto px-6 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {(skills.length > 0 ? skills.map(s => s.name) : defaultSkills).map((skill, i) => (
              <span key={i} className="px-6 py-3 rounded-xl bg-[#0e1011] border border-white/10 text-gray-300 text-sm font-medium hover:border-[#14a800] hover:text-[#14a800] hover:bg-[#14a800]/5 transition-all cursor-default">
                {skill}
              </span>
            ))}
          </div>
      ...
      <section className="py-32 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl font-bold mb-8 text-[#14a800]">{content?.aboutTitle || "About Me"}</h2>
