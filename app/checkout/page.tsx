...
export default function CheckoutPage() {
  ...
  // Handle Order Placement (Both Free and Paid)
  const handleSuccess = async (isPaidViaPayPal: boolean = false) => {
    try {
      ...
      await placeOrder({
        ...
        total: total,
        items: cart, // Pass cart items to backend
        couponCode: discountPercent > 0 ? couponCode : null,
        isPaid: isPaidViaPayPal // Pass payment status flag
      });

      toast.dismiss();
      toast.success("Order Placed Successfully!");
      clearCart(); // Empty the cart
      router.push('/thank-you'); // Redirect to success page
    } catch (error) {
      console.error(error);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {cart.length === 0 ? (
          <div className="text-center py-20 bg-[#0e1011] rounded-2xl border border-white/10">
            ...
            <Link href="/store" className="px-6 py-3 bg-[#14a800] text-white rounded font-bold hover:bg-[#118f00] transition">
              Browse Products
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div key={`${item.id}-${idx}`} className="flex gap-6 p-4 bg-[#0e1011] border border-white/10 rounded-xl items-center">
                  ...
                  <div className="text-right">
                    ...
                    <button 
                      ...
                      className="text-red-500 text-sm hover:underline flex items-center gap-1 mt-1 justify-end"
                    >
            ...
            <div className="bg-[#0e1011] p-8 rounded-2xl border border-white/10 h-fit sticky top-24">
              ...
              <div className="my-4 pt-4 border-t border-white/10">
                <div className="flex gap-2">
                  <input 
                    ...
                    className="flex-1 p-2 bg-white/5 border border-white/10 rounded text-sm text-white uppercase focus:border-[#14a800] outline-none"
                  />
                  <button 
                    onClick={handleApplyCoupon} 
                    className="bg-gray-800 text-white px-3 rounded text-sm hover:bg-gray-700 transition"
                  >
                    Apply
                  </button>
                </div>
                {couponMsg && (
                  <p className={`text-xs mt-1 ${discountPercent > 0 ? 'text-green-500' : 'text-red-500'}`}>
                    {couponMsg}
                  </p>
                )}
              </div>
              ...
              {formData.name && formData.email ? (
                <>
                  {total === 0 ? (
                    // OPTION A: Free Order Button (Manual Status = Pending)
                    <button 
                      onClick={() => handleSuccess(false)}
                      className="w-full bg-[#14a800] text-white py-4 rounded font-bold hover:bg-[#118f00] transition shadow-lg shadow-green-900/20"
                    >
                      Place Free Order
                    </button>
                  ) : (
                    // OPTION B: PayPal Buttons (Paid Status = Processing)
                    <PayPalScriptProvider options={{ 
                      clientId: "AcDbBJjcQINFFxHRLhVThmGS8t3C46YJnb45TEI6tTRBUiWpSnHCnJJKpJCk8tXDzK-_bi0WPp1C0s8n", 
                      currency: "USD" 
                    }}>
                      <PayPalButtons 
                        ...
                        onApprove={async (data, actions) => {
                          if (actions.order) {
                            ...
                            // Pass TRUE because this is a paid order
                            handleSuccess(true);
                          }
                        }}
                      />
                ...
                <button disabled className="w-full bg-gray-800 text-gray-500 py-3 rounded font-bold cursor-not-allowed">
